...
export function quickParseDeterministic(text) {
  ...
  if (
    firstLine &&
    !/req|property|m\.?v|n\.?o\.?b|itr|co-?applicant|remarks?/i.test(firstLine)
  ) {
    let nameLine = phoneMatch
      ? firstLine.replace(phoneMatch[0], "")
      : firstLine;
  ...
  out.loanAmount =
    grab(/req(?:uirement)?[\s.\-:]*([\d.]+\s*(?:cr|crore|lakh|lac|l)\b)/i) ||
    grab(/req(?:uirement)?[\s.\-:]*([\d.]+)/i);
  ...
  if (propMatch) {
    ...
    out.propertyType =
      PROPERTY_TYPES.find((p) => t.includes(p.toLowerCase())) || "";
  }

  out.marketValue = grab(
    /m\.?\s?v\.?[\s.\-:]*([\d.]+\s*(?:cr|crore|lakh|lac|l)?)/i
  );
  const nobMatch = text.match(/n\.?\s?o\.?\s?b\.?[\s.\-:]*([^\n]+)/i);
  if (nobMatch) {
    const t = nobMatch[1].toLowerCase();
    out.employment = /salar/i.test(t)
      ? "Salaried"
      : /business|shop|trade|rashan|kirana/i.test(t)
      ? "Self-Employed Business"
      : /profession|doctor|ca\b|architect/i.test(t)
  ...
  if (foundBank) {
    out.existingLoanBank = foundBank;
    out.bank = foundBank;
  }

  if (/\blap\b/i.test(text))
    out.product = /\bbt\b|balance transfer/i.test(text)
      ? "LAP Balance Transfer (LAP BT)"
      : "Loan Against Property (LAP)";
  else if (/home loan|\bhl\b/i.test(text))
    out.product = /\bbt\b|balance transfer/i.test(text)
      ? "HL Balance Transfer (HL BT)"
      : "Home Loan (HL)";
  else if (/msme/i.test(text)) out.product = "MSME Working Capital";
...
export function leadsToCSV(leads) {
  const cols = [
    "name", "phone", "altPhone", "product", "bank", "loanAmount", "status",
    "reason", "interest", "location", "propertyType", "propertyLocation",
    "marketValue", "employment", "itr", "coApplicant", "existingLoanBank",
    "turnover", "bankingType", "rentalIncome", "monthlySalary",
    "companyCategory", "nextCallDate", "nextCallTime", "notes",
  "meetingsDone", "loginsDone",
  "convertedAt",
  ];
...
export function buildGeminiPrompt(lead) {
  const lines = [
    ...
    `Employment / Nature of Business: ${lead.employment || "N/A"}`,
    ...
    `Rental Income: ${lead.rentalIncome || "N/A"}`,
  ...
  return `You are a senior Indian loan advisor (DSA) sales coach. Analyze this lead and respond ONLY in Hinglish (Hindi+English mix, roman script). Keep it practical and field-ready.\n\n${lines.join("\n")}\n\nGenerate exactly 4 sections, each on a new line, with these exact labels:\n1) Leverage: <one line core psychological leverage point>\n2) Hook: <5-8 second high-conversion opening hook, in quotes>\n3) Objection Destroyer: <one line comeback script>\n4) Cross-sell: <one line cross-sell / product bridge strategy>`;
}
...
export function parseCopilotResponse(text) {
  const sections = { leverage: "", hook: "", objection: "", crossSell: "" };
  const lines = text.split("\n").map((l) => l.trim()).filter(Boolean);
  for (const line of lines) {
    const m = line.match(/^\s*(?:\d+\))?\.?\s*(Leverage|Hook|Objection(?:\s*Destroyer)?|Cross[- ]?sell)\s*[:\-]\s*(.*)/i);
    if (m) {
