...
export function buyingIntentScore(lead) {
  ...
  const fields = [
    ...
    lead.cibilScore,
    lead.currentROI,
    lead.entityConstitution,
    lead.natureOfBusiness,
    lead.additionalIncome,
...
// and dumps anything unparsed into Notes.
export function quickParseDeterministic(text) {
  const out = {};
  const consumed = []; // track matched substrings to build leftover notes
  ...
  if (firstLine && !/req|property|m\.?v|n\.?o\.?b|itr|co-?applicant|remarks|cibil|roi|bank|salary|turnover/i.test(firstLine)) {
  ...
  const existLoanMatch = text.match(/existing\s*loan[\s.\-:]*([^\n,]+)/i);
  if (existLoanMatch) {
    out.existingLoanRemarks = existLoanMatch[1].trim();
  ...
  const salaryMatch = text.match(/salary[\s.\-:]*([\d.]+)/i)
    || text.match(/monthly\s*income[\s.\-:]*([\d.]+)/i);
  if (salaryMatch) {
  ...
  if (/not interested/i.test(text)) {
    ...
    mark(text.match(/not interested/i)?.[0] || "");
  ...
  // remove pure-punctuation fragments
...
export function leadsToCSV(leads) {
  const cols = [
    ...
    "marketValue", "employment", "entityConstitution", "natureOfBusiness",
    "turnover", "bankingType", "itr", "cibilScore", "currentROI",
    "monthlySalary", "companyCategory", "additionalIncome", "additionalIncomeAmt",
...
export function buildGeminiPrompt(lead) {
  ...
  const lines = [
    ...
    `Existing Loan Details: ${lead.existingLoanRemarks || "N/A"}`,
