...
// Normalize any date string (YYYY-MM-DD or YYYY/MM/DD) to YYYY/MM/DD for
// reliable local-time comparison regardless of separator.
...
export function fmtDateTime(dateStr, timeStr) {
  ...
  // Date constructor needs ISO-style hyphens; normalize slashes away.
  const iso = String(dateStr).replace(/\//g, "-");
  return new Date(`${iso}T${timeStr || "09:00"}`);
}

export function urgency(lead) {
  if (!lead.nextCallDate) return "none";
  const dt = fmtDateTime(lead.nextCallDate, lead.nextCallTime);
  if (!dt || isNaN(dt.getTime())) return "none";
  const diffHrs = (dt - new Date()) / 3600000;
  if (diffHrs < 0) return "overdue";
  // "today" = same calendar date in local time, regardless of separator.
  if (normalizeDate(lead.nextCallDate) === todayStr()) return "today";
